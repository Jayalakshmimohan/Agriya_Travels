...
/**
 * The API half of the app: security headers, rate limits and every /api route.
 *
 * Extracted from server.ts so it can run in two very different places without
 * being defined twice. Locally, server.ts wraps it with Vite middleware and
 * MPA routing and listens on a port. On Vercel, api/index.ts exports it as a
 * serverless handler and Vercel's CDN serves the static build instead.
 *
 * Deliberately contains no static file serving, no Vite and no listen() — those
 * are the parts that differ between the two environments.
 */
